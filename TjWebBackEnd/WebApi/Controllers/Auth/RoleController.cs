    ...
    public class RoleController : ApiController
    {
        ...
        public IHttpActionResult List(RoleRequestPayload payload)
        {
            ...
            using (_dbContext)
            {
                ...
                var list = query.OrderByDescending(x => x.CreatedOn).Paged(payload.CurrentPage, payload.PageSize)
                    .ToList();
        ...
        public IHttpActionResult Edit(RoleCreateViewModel model)
        {
            ...
            using (_dbContext)
            {
                ...
                if (_dbContext.Roles.Any(x => x.Name == model.Name && x.Code != model.Code))
                {
                    response.SetFailed("角色已存在");
                    return Ok(response);
                }
